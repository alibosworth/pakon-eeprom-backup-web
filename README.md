...
3. Read the calibration EEPROM with the OEM engine's own two requests
   ...
   Then read the boot chip itself (`0x51`), which is the same two requests
   with `0x00A3` in the select. Its first 8 bytes are a Cypress C0 boot
   record, `c0 05 0f 35 f2 07 aa 04`, which is the personality proper and is
   exactly what the loader reports; a ninth byte `02` follows it whose purpose
   is not known, then `0xFF` padding, with no length header and no CRC. This one is read last and its failure is not
...
4. Download everything as one zip (built in the page): the four section
   files, the boot chip, the loader's 8-byte personality if step 2 ran, a `SHA256SUMS`,
