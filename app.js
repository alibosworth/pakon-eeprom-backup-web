...
function showPlatformNotice() {
  ...
  if (!info.hasWebUsb) {
    ...
    html = "<strong>Windows: this page will most likely not see your scanner.</strong> The Pakon driver keeps the scanner to itself, and giving Chrome access means replacing that driver, which stops the Pakon software working until you put it back. Easiest is to run this page from a Mac, or from a Linux live USB booted on this PC, with Chrome; nothing gets installed either way. (The Kodak software's copy in the registry is not a real backup: it is a decoded, rounded copy that cannot be written back.)" + localTools;
